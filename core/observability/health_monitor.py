...
class HealthMonitor:
    ...
    def record_ingestion(self, *, latency_ms: float) -> None:
        self._last_ingestion_at = datetime.now(tz=timezone.utc)
    ...
    def snapshot(self) -> HealthSnapshot:
        ...
        hist = s["histograms"]
        diag_mean = float(hist.get("diagnosis_latency_ms", {}).get("mean", 0.0))
        ing_mean = float(hist.get("ingestion_latency_ms", {}).get("mean", 0.0))
        orch_mean = float(hist.get("orchestration_latency_ms", {}).get("mean", 0.0))
